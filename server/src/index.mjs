...
server.listen(port, function () {
	log.info('webserver listens on port ', port)
})
...
io.on('connection', function (socket) {
	...
	log.debug(`New connection with id ${socket.id}, with parameters '${JSON.stringify(socket.handshake.query)}'`)
	...
	socket.on('disconnecting', function () {
		log.debug(`Connection with id ${socket.id} left (alias: ${socket.alias}, room: ${socket.roomId})`)
...
function joinRoom(socket, roomId) {
	...
	log.debug(`${socket.alias} joined ${roomId}`)
}
...
function sendToPeers(socket, event, data) {
	...
	log.debug(`${socket.alias} broadcasts event ${event}: ${JSON.stringify(data)}`)
